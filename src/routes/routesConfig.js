...
// Function to broadcast event via WebSocket (replacement for Firestore)
...
async function protectedRoutes(fastify, options) {
  ...
  fastify.post("/api/notification/send-to-group", async (request, reply) => {
    try {
      const { userId, notification, data } = request.body;
      ...
      const result = await sendNotificationToUserGroup(
        userId,
        notification,
        data || {}
      );
  ...
  fastify.post("/api/notification/send-with-group", async (request, reply) => {
    try {
      const { userGroups, notification, data } = request.body;
      ...
      // Kirim notifikasi ke grup
      const result = await sendNotificationToGroup(
        userGroups,
        notification,
        data || {}
      );
