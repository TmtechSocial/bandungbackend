...
async function fetchApiData(
  ...
  try {
    ...
    const {
      ...
      exclude = {}, // support exclude parameters
    ...
    // Process exclude parameters - Support multiple exclude fields
    if (exclude && Object.keys(exclude).length > 0) {
      console.log(`[fetchApiData] Processing ${Object.keys(exclude).length} exclude parameters:`, Object.keys(exclude));
      
      Object.entries(exclude).forEach(([fieldKey, fieldTemplate]) => {
        if (typeof fieldTemplate === "string") {
          // Support multiple template variables in exclude values
          const processedValue = fieldTemplate.replace(/\$\{([^}]+)\}/g, (match, templateVar) => {
            const resolvedValue = substitutionData[templateVar] ?? "";
            console.log(`[fetchApiData] Exclude "${fieldKey}" template "${match}" → "${resolvedValue}"`);
            return resolvedValue;
          });
          
          // Handle multiple values separated by comma
          if (processedValue.includes(',')) {
            const excludeParam = `${fieldKey}:${processedValue}`;
            console.log(`[fetchApiData] Exclude param "${fieldKey}": multiple values → "${excludeParam}"`);
            processedParams['exclude'] = processedParams['exclude'] 
              ? `${processedParams['exclude']},${excludeParam}` 
              : excludeParam;
          } else if (processedValue) {
            const excludeParam = `${fieldKey}:${processedValue}`;
            console.log(`[fetchApiData] Exclude param "${fieldKey}": single value → "${excludeParam}"`);
            processedParams['exclude'] = processedParams['exclude'] 
              ? `${processedParams['exclude']},${excludeParam}` 
              : excludeParam;
          }
        } else if (Array.isArray(fieldTemplate)) {
          // Handle array of exclude values
          const processedValues = fieldTemplate.map(val => {
            if (typeof val === "string") {
              return val.replace(/\$\{([^}]+)\}/g, (match, templateVar) => {
                return substitutionData[templateVar] ?? "";
              });
            }
            return val;
          ...
          if (processedValues.length > 0) {
            const excludeParam = `${fieldKey}:${processedValues.join(',')}`;
            console.log(`[fetchApiData] Exclude param "${fieldKey}": array values → "${excludeParam}"`);
            processedParams['exclude'] = processedParams['exclude'] 
              ? `${processedParams['exclude']},${excludeParam}` 
              : excludeParam;
          }
      ...
      if (processedParams['exclude']) {
        console.log(`[fetchApiData] Final exclude parameter: "${processedParams['exclude']}"`);
      }
    }
