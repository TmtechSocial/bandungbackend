...
function getApiDependencies(config) {
  ...
  // Check exclude parameters for API dependencies
  if (config.exclude) {
    Object.values(config.exclude).forEach(value => {
      if (Array.isArray(value)) {
        value.forEach(extractApiDeps);
      } else {
        extractApiDeps(value);
      }
    });
  }
...
function resolveApiConfigs(apiConfigs, processedQueryData) {
  ...
  for (const [key, config] of Object.entries(apiConfigs)) {
    ...
    // Handle 'exclude' params - support multiple 'exclude' parameters
    if (config.exclude) {
      const resolvedConfig = { ...config };
      if (typeof resolvedConfig.url === "string") {
        resolvedConfig.url = resolveTemplate(
      ...
      console.log(`[resolveApiConfigs] Processing ${Object.keys(config.exclude).length} 'exclude' parameters for ${key}:`, Object.keys(config.exclude));
      
      // Process exclude parameters
      const processedExclude = {};
      for (const [fieldKey, fieldTemplate] of Object.entries(config.exclude)) {
        if (typeof fieldTemplate === "string") {
          const resolvedValue = resolveTemplate(fieldTemplate, processedQueryData);
          processedExclude[fieldKey] = resolvedValue;
          console.log(`[resolveApiConfigs] 'exclude' param "${fieldKey}": template "${fieldTemplate}" → "${resolvedValue}"`);
        } else if (Array.isArray(fieldTemplate)) {
          const resolvedValues = fieldTemplate.map(val => {
            if (typeof val === "string") {
              return resolveTemplate(val, processedQueryData);
            }
            return val;
          }).filter(val => val); // Remove empty values
          processedExclude[fieldKey] = resolvedValues;
          console.log(`[resolveApiConfigs] 'exclude' param "${fieldKey}": array template → [${resolvedValues.join(',')}]`);
        } else {
          processedExclude[fieldKey] = fieldTemplate;
        }
      }
      
      resolvedConfig.exclude = processedExclude;
      resolvedConfigs[key] = resolvedConfig;
      console.log(`[resolveApiConfigs] ${key} resolved with ${Object.keys(processedExclude).length} 'exclude' params:`, Object.keys(processedExclude));
      continue;
    }
