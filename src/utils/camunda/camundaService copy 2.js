...
import("camunda-external-task-client-js").then(
  ({ Client, logger, Variables }) => {
    ...
    client.subscribe("Mutasi_Inventory",
  ...
  insert_mo_refill_print(objects: {mo_refill_id: $mo_refill_id, sku_toko: $sku_toko, sku_gudang: $sku_gudang, gudang: $gudang, refill_type: $refill_type, proc_inst_id: $proc_inst_id, created_at: $created_at}) {
    ...
    client.subscribe("Recomendation_SimpleAssembly",
      async function ({ task, taskService }) {
    ...
    client.subscribe("testingFunction", async function ({ task, taskService }) {
      ...
      try {
        ...
        console.log(location_id, stock_id, part_id, quantity);
                ...
                const stockTransfer = await transferStock( stock_id, quantity, location_id, notesTransfer);
    ...
    client.subscribe("Complete_Build_Partial",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
    ...
    client.subscribe("Get_Instance_Prepare",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
    ...
    client.subscribe("getInstanceRetur",
      async function ({ task, taskService }) {
        ...
        try {
          ...
          console.log(
            "📊 Response Hasura:",
            JSON.stringify(responseUpdate.data, null, 2)
          );
    ...
    client.subscribe("generate_stock_opname_komponen",
    ...
    client.subscribe("ConvertQuantityInbound",
      async ({ task, taskService }) => {
    ...
    client.subscribe("getInstanceClosing",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
        try {
          const proc_inst_id = task.processInstanceId;
    ...
    client.subscribe("getInstanceGenericStaging",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
    ...
    client.subscribe("getToleransiPartStaging",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
        try {
          const source_stock = task.variables.get("source_stock");
    ...
    client.subscribe("transferStockStagingArea",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
        try {
          const proc_inst_id = task.processInstanceId;
    ...
    client.subscribe("Check_Stock_Mutasi_Prepare",
      async function ({ task, taskService }) {
        try {
          console.log(
            `🕒 Task "${task.topicName}" diterima (${task.id}) — menunggu 10 detik sebelum eksekusi...`
    ...
    client.subscribe("Check_Stock_Prepare",
      async function ({ task, taskService }) {
        try {
          const inventree = axios.create({
            baseURL: `${process.env.SERVER_INVENTREE}/api`,
    ...
    client.subscribe("checkQuantityWIP",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
    ...
    client.subscribe("getInstanceMutasiPrepare",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
    ...
    client.subscribe("getInstanceInternalPrepare",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
        try {
          const proc_inst_id = task.processInstanceId;
    ...
    client.subscribe("getInstanceInbound",
      async function ({ task, taskService }) {
        console.log("🚀 Task Dijalankan:", task.id);
        try {
          const proc_inst_id = task.processInstanceId;
