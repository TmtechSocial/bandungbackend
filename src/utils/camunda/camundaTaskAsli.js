  ...
  WITH task_variables AS (
    SELECT 
        v.proc_inst_id_,
         string_agg(
            CASE 
                WHEN v.name_ = 'business_key' THEN v.text_
                ELSE NULL 
            END,
            ''
        ) as business_key
    FROM public.act_ru_variable v
    WHERE v.name_ IN ('business_key')
    GROUP BY v.proc_inst_id_
),
identitylink_grouped AS (
    SELECT 
        task_id_,
        string_agg(COALESCE(group_id_, user_id_), ', ') AS delegated
    FROM public.act_ru_identitylink
    GROUP BY task_id_
)
SELECT DISTINCT
...
module.exports.fetchAllTasks = async (
  ...
  try {
	...
	console.log('returnnnnnnnnnnnnnnnn',returnRaw)
    if (returnRaw) {
