...
function getNestedData(obj, path) {
  ...
  // console.log(
  //   `[getNestedData] Searching for path: ${path} in:`,
  //   Object.keys(obj)
  // );
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    // console.log(
    //   `[getNestedData] Step ${i}: Looking for key "${key}" in:`,
    //   typeof current,
    ...
    if (current && typeof current === "object") {
      if (key in current) {
        current = current[key];
        // console.log(
        //   `[getNestedData] Step ${i}: Found "${key}", type:`,
        //   typeof current,
        ...
        if (Array.isArray(current) && i < keys.length - 1) {
          if (current.length > 0) {
            // console.log(
            //   `[getNestedData] Step ${i}: Entering first element of array for further navigation (${
            //     keys.length - i - 1
            //   } steps remaining)`
            // );
            current = current[0];
            ...
            if (!current || typeof current !== "object") {
                // console.log(
                //   `[getNestedData] Step ${i}: Array element is not navigable:`,
                //   typeof current
                // );
                // console.log(
                //   `[getNestedData] Navigation history:`,
                //   navigationHistory
                // );
              return null;
            }
            ...
            // console.log(
            //   `[getNestedData] Step ${i}: Array is empty, cannot navigate further`
            // );
            // console.log(
            //   `[getNestedData] Navigation history:`,
            //   navigationHistory
        ...
        // console.log(
        //   `[getNestedData] Available keys at step ${i}:`,
        //   Object.keys(current)
        // );
      ...
      // console.log(
      //   `[getNestedData] Step ${i}: Current is not an object:`,
      //   current
      // );
      // console.log(`[getNestedData] Navigation history:`, navigationHistory);
      return null;
  ...
  // console.log(
  //   `[getNestedData] SUCCESS - Final result for path ${path}:`,
  //   typeof current,
...
function processDataGridComponent(component, queryData, formState) {
  ...
  if (formState && formState.apiResults) {
    console.log(
      "[datagrid] API Results available keys:"
    );
  } else {
    console.warn("[datagrid] API Results belum tersedia!");
  }

  // Proses content component terlebih dahulu, simpan hasil HTML berdasarkan key (for rendering only, not for data)
...
>Lihat Evidence</a>`;
  ...
  function findLongestCommonPrefix(paths) {
    ...
    paths.forEach((path) => {
      ...
      if (relatedPaths.length > 1) {
        ...
        // console.log(
        //   `[datagrid] Grouped paths under "${primaryPath}":`,
        //   relatedPaths
        // );
      } else {
  ...
  // console.log(
  //   `[datagrid] Final primary path groupings:`,
  //   Object.fromEntries(
  ...
  // console.log(
  //   "[datagrid] Table component mapping:",
  //   Object.keys(tableComponentMap)
  // );
  // console.log(
  //   `[datagrid] hasTableComponents: ${hasTableComponents}, hasApiComponents: ${hasApiComponents}`
  // );
  ...
  if (queryData && queryData.length > 0) {
    queryData.forEach((queryItem, qIndex) => {
      if (queryItem.graph) {
        console.log(
          `[datagrid] Query ${qIndex} graph structure:`
        );
      }
    });
  }
  ...
  if (!hasTableComponents && hasApiComponents && formState?.apiResults) {
    // console.log(
    //   `[datagrid] 🔄 SPECIAL CASE: No table components, creating rows from API data`
    // );
    ...
    for (const source of priorityOrder) {
      if (uniqueApiSources.includes(source)) {
        ...
        if (Array.isArray(apiData) && apiData.length > 0) {
          ...
          // console.log(
          //   `[datagrid] ✅ Using priority API source: ${source} with ${apiData.length} items`
          // );
          break;
        }
      }
    ...
    if (!primaryApiSource) {
      for (const source of uniqueApiSources) {
        ...
        if (Array.isArray(apiData) && apiData.length > 0) {
          ...
          primaryApiData = apiData;
          // console.log(
          //   `[datagrid] ✅ Using fallback API source: ${source} with ${apiData.length} items`
          // );
          break;
        }
      }
    ...
    if (primaryApiSource && primaryApiData) {
      ...
      primaryApiData.forEach((apiItem, index) => {
        ...
        // console.log(
        //   `[datagrid] API Row ${index}: Created from ${primaryApiSource}, fields:`,
        //   Object.keys(row).filter((k) => k !== "image_preview")
        // );
    ...
    queryData.forEach((queryItem) => {
      Object.entries(tableComponentMap).forEach(
        ([primaryPath, tableComponents]) => {
          ...
          // console.log(
          //   `[datagrid] Related table paths:`,
          //   Array.from(primaryTablePath[primaryPath])
          // );

          // Handle SQL data (direct table match only for SQL)
          if (queryItem.sqlQuery?.table === primaryPath) {
            // console.log(
            //   `[datagrid] Processing SQL data for table: ${primaryPath}, rows: ${queryItem.sqlQuery.data.length}`
            // );
            queryItem.sqlQuery.data.forEach((item, index) => {
              ...
              // console.log(
              //   `[datagrid] SQL Row ${index}: ${item.product_name?.substring(
              //     0,
              //     40
              //   )}..., fields saved:`,
              //   Object.keys(row).filter((k) => k !== "image_preview")
          ...
          if (queryItem.graph) {
            ...
            if (primaryPath.includes(".")) {
              ...
              if (primaryData && Array.isArray(primaryData)) {
                // console.log(
                //   `[datagrid] ✅ FOUND primary nested graph data for path: ${primaryPath}, rows: ${primaryData.length}`
                // );
                processedData = true;
                ...
                primaryData.forEach((primaryItem, index) => {
                  ...
                  tableComponents.forEach((sc) => {
                    ...
                    if (componentTablePath.startsWith(primaryPath + ".")) {
                      ...
                      // console.log(
                      //   `[datagrid] Processing deeper nested path: ${extraPath} for component ${sc.key}`
                      // );
                  ...
                  // console.log(
                  //   `[datagrid] Enhanced Nested Row ${index}: fields saved:`,
                  //   Object.keys(row).filter((k) => k !== "image_preview")
                  // );
                  newDefaultValue.push(row);
                });
                ...
                // console.log(
                //   `[datagrid] ✅ FOUND primary nested graph object for path: ${primaryPath}`
                // );
                processedData = true;
                ...
                tableComponents.forEach((sc) => {
                  ...
                  const componentTablePath = sc.table;
                  if (
                    componentTablePath !== primaryPath &&
                    componentTablePath.startsWith(primaryPath + ".")
                  ) {
                    const extraPath = componentTablePath.substring(
                      primaryPath.length + 1
                    );
                ...
                // console.log(
                //   `[datagrid] Enhanced Nested Object: fields saved:`,
                //   Object.keys(row).filter((k) => k !== "image_preview")
                // );
                newDefaultValue.push(row);
              }
            ...
            if (!processedData && queryItem.graph[primaryPath]) {
              // console.log(
              //   `[datagrid] 📌 Using direct graph data for table: ${primaryPath}, rows: ${queryItem.graph[primaryPath].length}`
              // );
              queryItem.graph[primaryPath].forEach((item, index) => {
                ...
                // console.log(
                //   `[datagrid] Direct Graph Row ${index}: ${item.product_name?.substring(
                //     0,
                //     40
                //   )}..., fields saved:`,
                //   Object.keys(row).filter((k) => k !== "image_preview")
            ...
            if (!processedData && !queryItem.graph[primaryPath]) {
              // console.log(
              //   `[datagrid] ❌ No data found for primary path: ${primaryPath}`
              // );
              // console.log(
              //   `[datagrid] Available graph keys:`,
              //   Object.keys(queryItem.graph)
              // );

              // Debug: Show the actual nested structure
              const debugNestedData = getNestedData(
                queryItem.graph,
                primaryPath
              );
              // console.log(
              //   `[datagrid] Debug nested data for ${primaryPath}:`,
              //   debugNestedData
              // );
            }
          }
  ...
  if (Object.keys(apiComponentMap).length > 0 && newDefaultValue.length > 0) {
    // console.log(
    //   `[datagrid] Query rows: ${newDefaultValue.length}, API data available: ${
    //     Object.keys(formState.apiResults || {}).length
    //   }`
    // );
    ...
    Object.entries(apiComponentMap).forEach(([source, apiComponents]) => {
      ...
      // console.log(
      //   `[datagrid] Processing API source [${source}]:`,
      //   Array.isArray(apiItems1)
      //     ? `Array(${apiItems1.length})`
      //     : typeof apiItems1
      // );
      ...
      // console.log(
      //   `[datagrid] API source [${source}]: ${
      //     isValidApiData ? `Array(${items.length})` : "Invalid/Empty"
      //   }`
      // );

      if (isValidApiData) {
        // console.log(
        //   `[datagrid] Mapping ${newDefaultValue.length} rows with ${items.length} API items from source: ${source}`
        // );
        ...
        newDefaultValue.forEach((row, rowIndex) => {
          ...
          // console.log(
          //   `[datagrid] Row ${rowIndex}: Product="${
          //     row.product_name?.substring(0, 30) ||
          //     row.full_name?.substring(0, 30) ||
          //     "Unknown"
          //   }...", Match: ${matchMethod || "NONE"}, API found: ${!!apiItem}`
          // );
          ...
          apiComponents.forEach((sc) => {
            ...
            if (sc.key === "image") {
              ...
              if (
                ...
                // console.log(
                //   `[datagrid] Row ${rowIndex}: Image updated from "${
                //     row[sc.key]
                //   }" to "${apiValue}" via ${matchMethod}`
                // );
                row[sc.key] = apiValue;
              } else {
                console.log(
              ...
              if (apiValue !== undefined && apiValue !== null) {
                // console.log(
                //   `[datagrid] Row ${rowIndex}: Field ${sc.key} updated to "${apiValue}" via ${matchMethod}`
                // );
                row[sc.key] = apiValue;
              }
            }
  ...
  if (
    ...
    component.defaultValue.forEach((row) => {
      ...
      // Do NOT overwrite any data field with html, only use contentHTMLMap for rendering in the frontend
  ...
  // console.log(
  //   `[datagrid] Final result: ${newDefaultValue.length} rows processed`
  // );
