  ...
  async onSubmit(data, process) {
    ...
    for (const item of data) {
      try {
        ...
        // Ambil lokasi source tiap product dari Inventree API + cek stok
for (const product of products) {
  try {
    ...
    const stockQty = stockResponse.data?.quantity || 0;
    const locationName = stockResponse.data?.location_name || "UnknownLocation";

    product.source_location_name = locationName;
    product.available_stock = stockQty; // simpan untuk logging/debug

    console.log(
      `📦 SKU ${product.source_sku} | Location: ${locationName} | Stock: ${stockQty} | Requested: ${product.quantity}`
    );

    // ✅ Cek apakah stok cukup   
    if (stockQty < product.quantity) {
      throw new Error(
        `Stock tidak cukup untuk SKU ${locationName}. Dibutuhkan ${product.quantity}, tersedia ${stockQty}`
      );
    }

  } catch (err) {
    // Kalau error ambil stok, tetap kasih info default
    product.source_location_name = "UnknownLocation";
    product.destination_location_name = "UnknownLocation";
    throw new Error(
      `Gagal cek stock untuk SKU ${product.source_sku}: ${err.message}`
    );
  }
}


        const businessKeyValue = products
  .map((p) => p.source_location_name || "UnknownLocation")
  .join(":");

console.log("🆔 Business Key (gabungan):", businessKeyValue);
	...
	const dataCamunda = {
          ...
          variables: {
            variables: {
              ...
              business_key: { value: businessKeyValue, type: "String" }
            },
          },
        };
