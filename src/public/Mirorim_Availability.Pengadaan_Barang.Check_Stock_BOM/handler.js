  ...
  async onSubmit(data, process) {
    ...
    for (const item of data) {
      try {
        ...
        const componentPk = item.products.map(product => product.pk);
        ...
        const dataCamunda = {
          ...
          variables: {
            variables: {
              component_enough: { value: item.component_enough, type: "boolean"},
              report_valid : { value: item.component_enough, type: "boolean"},
              quantity_request: { value: item.quantity_request, type: "Integer" },
              component_pk: { value: componentPk, type: "String" },
            },
          },
        };
          ...
          const dataQuery = {
            graph: {
              ...
              variables: {
                ...
                status: item.component_enough ? 'in_progress' : 'rejected_components',
                note: item.note || ''
              },
            },
            query: [],
