      ...
      try {
        ...
        for (const member of item.team_members) {
          if (member.employeeType === "Staff") {
            // Worker: format move (exclude PKL dari before)
            const groupNameArray = member.groupName
              .split(",")
              .map((g) => g.trim())
              .filter((g) => g !== "" && g !== "PKL" && g !== "mirorim"); // Filter empty strings, PKL, and mirorim
            ...
            // Non-worker (coordinator, leader, manager, etc): format add/remove
            const groupNameArray = member.groupName
              .split(",")
              .map((g) => g.trim())
            ...
            // groupAdd: yang ada di updateGroupMulti
            const groupAdd = updateGroupArray.join(", ");

            // groupRemove: yang ada di groupName tapi tidak ada di updateGroupMulti
            const groupRemove = groupNameArray
              .filter((g) => !updateGroupArray.includes(g))
              .join(", ");
