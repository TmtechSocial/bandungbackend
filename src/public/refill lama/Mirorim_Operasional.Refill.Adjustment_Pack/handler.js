  ...
  async onSubmit(data, process) {
    ...
    for (const item of data) {
      try {
        ...
        const adjustValue =
          item.adjust_type === "add"
            ? item.quantity_approve + item.quantity_adjust
            : item.quantity_approve - item.quantity_adjust;
        ...
        if (responseCamunda.status === 200 || responseCamunda.status === 204) {
          ...
          const dataQuery = {
            graph: {
              ...
              gqlQuery: `mutation MyMutation($proc_inst_id: String!, $quantity_adjust: Int!, $adjust_type: String!, $quantity_approve: Int!) {
      update_mo_refill(
