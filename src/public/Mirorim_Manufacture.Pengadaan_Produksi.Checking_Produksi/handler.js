  ...
  async onSubmit(data, process) {
    ...
    for (const item of data) {
      try {
        ...
        const quantityRequest = item.products.map(product => product.quantity);
        const quantityChecks = item.products.map(product => product.quantity_check);

        console.log("Isi quantity_request:", quantityRequest);
        ...
        const dataCamunda = {
          ...
          variables: {
            variables: {
              quantity_request: { value: quantityRequest, type: "String" },
          ...
          const dataQuery = item.products.map(product => ({
            graph: {
              ...
              variables: {
                ...
                source: "QC Order Manufacture",
                status:
                  product.quantity_check !== product.quantity || Boolean(product.action)
                    ? "Failed QC"
                    : "Done QC",
              },
            },
