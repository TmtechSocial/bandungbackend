      ...
      if (item.kegiatan == "alokasiResource") {
        ...
        try {
          ...
          for (const member of item.team_members) {
            ...
            if (member.employeeType === "Staff") {
              ...
              // Step 3: DELETE dari semua group yang ada (kecuali PKL)
          ...
          console.error(
            `âŒ Error executing handler for event: ${eventKey}`,
            error
          );
          console.error(
            "Error details:",
        ...
        try {
          ...
          for (const apr of rejectedMembers) {
            console.log(
              `\nâŒ Processing REJECT for user: ${apr.user} - Skip LDAP operations`
            );
          ...
          console.error(
            `âŒ Error executing handler for event: approvalAlokasi`,
            error
          );
          console.error(
            "Error details:",
