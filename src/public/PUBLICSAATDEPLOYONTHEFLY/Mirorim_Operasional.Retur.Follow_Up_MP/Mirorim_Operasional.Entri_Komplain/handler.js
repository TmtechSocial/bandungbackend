      ...
      try {
        ...
        const checkQuery = {
          graph: {
            ...
            gqlQuery: `
      query CheckResi($invoice: String!) {
        mo_order_logs(where: {invoice: {_eq: $invoice}}) {
          id
        }
      }
    `,
        ...
        const moRetur = checkData.mo_order_logs || [];

        console.log("checkData.mo_order_logs", checkData.mo_order_logs);
        console.log("item.invoice", item.invoice);
        console.log("moRetur.length", moRetur.length);
        if (moRetur.length > 0) {
          throw new Error(`Invoice ${item.invoice} sudah di entri komplain`);
        }
