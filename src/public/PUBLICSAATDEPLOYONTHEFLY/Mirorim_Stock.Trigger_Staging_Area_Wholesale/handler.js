  ...
  async onSubmit(data, eventKey) {
    ...
    for (const item of data) {
      try {
        ...
        const dataCamunda = {
          type: "start",
          endpoint: `/engine-rest/process-definition/key/Mirorim_Stock.Trigger_Staging_Area_Wholesale/start`,
          variables: {
            variables: {
          ...
          for (const product of item.products) {
            ...
            for (const [typeKey, group] of Object.entries(groupedByType)) {
              ...
              const transferPayload = {
                ...
                notes: `Transfer WIP Staging Wholesale | Proc ID: ${item.proc_inst_id}`,
                location: 1000002,
              };
              ...
              const { data: getData } = await inventree.get(
                `/stock/?location=1000002&part=${item.part_pk}&ordering=-updated&limit=1`
              );
